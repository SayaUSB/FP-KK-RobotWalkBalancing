...
# Thresholds for fall detection parameters
FALL_COM_Z_THRESHOLD = 0.3  # If CoM under the threshold counts as fall
FALL_TRUNK_ANGLE_THRESHOLD = 0.5   # radian (≈28.6°). If roll/pitch under the threshold counts as fall
FALL_COM_DROP_THRESHOLD = 0.05 
...
parameters.walk_max_dx_forward = 1 # Maximum dx per step forward [m]
...
def rot_to_rpy(R):
    """Matrix rotation into euler (roll, pitch, yaw)"""
    r = np.arctan2(R[2, 1], R[2, 2])                                # roll  = atan2(R[2,1], R[2,2])
    p = np.arctan2(-R[2, 0], np.sqrt(R[2, 1] ** 2 + R[2, 2] ** 2))  # pitch = atan2(-R[2,0], sqrt(R[2,1]^2 + R[2,2]^2))
    y = np.arctan2(R[1, 0], R[0, 0])                                # yaw   = atan2(R[1,0], R[0,0])
    return r, p, y                                          
...
while True:
    ...
    if args.pybullet:
        com_pos, _ = p.getBasePositionAndOrientation(1)  # robot_id dari PyBullet
        com_z = com_pos[2]
        logger.info(f"CoM={com_z}")
    ...
    R_trunk = None
    try:
        T_trunk = robot.get_T_world_trunk()   # 4x4 transform, jika ada
        R_trunk = T_trunk[:3, :3]
    except Exception:
        try:
            R_trunk = robot.get_R_world_trunk()   # mungkin mengembalikan 3x3
        except Exception:
            # fallback ke trajectory (bila tersedia)
            try:
                R_trunk = trajectory.get_R_world_trunk(t)
            except Exception:
                # jika semua gagal, set identity (tidak ada deteksi sudut)
                R_trunk = np.eye(3)

    roll, pitch, yaw = rot_to_rpy(R_trunk)

    # Fall check
    if com_z < FALL_COM_Z_THRESHOLD:
        print("=== FALL DETECTED ===")
        time.sleep(1)
        # ---- tindakan saat jatuh terdeteksi ----
        # Pilihan:
        #  - hentikan control loop:
        #       break
        #  - hentikan replanning/motor command, tetap display:
        #       do_recovery_procedure()  # (implementasikan sendiri)
        #
        # Sekarang saya hentikan loop agar simulasi tidak terus mencoba jalan.
        # Ganti sesuai kebutuhan (misal: panggil policy recovery, kirim perintah safety, dll).
        break
