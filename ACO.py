...
def get_next_run_directory(base_dir="log/aco"):
...
def plot_optimization_results(history, memory_history, save_path):
    """
    Plot ACO convergence history AND Memory Usage over Time
    """
    ...
    plot_file = os.path.join(save_path, 'aco_analysis_report.png')
...
def main():
    ...
    current_log_dir = get_next_run_directory("log/aco")
    ...
    tuner.save_results(os.path.join(current_log_dir, 'aco_results.json'))
    walk_params.save_to_file(os.path.join(current_log_dir, 'best_walk_params_aco.json'))
    
    # 6. Plot results (Pass history AND memory_history)
    print("\nPlotting results...")
